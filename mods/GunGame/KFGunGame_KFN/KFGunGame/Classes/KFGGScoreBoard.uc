...
function DrawTitle(Canvas Canvas, float HeaderOffsetY, float PlayerAreaY, float PlayerBoxSizeY)
{
	...
	TitleString = GameTypeTitleString @ "|" @ MaxWeaponLevelString$":"@KFGGGameReplicationInfo(GRI).MaxWeaponLevel@"|"@Level.Title;
...
simulated final function DrawTeamScores( byte Team, Canvas Canvas, float XOfs )
{
	...
	if( Team==0 )
		Canvas.DrawColor = Class'Hud'.Default.RedColor;//GreenColor;
	else 
