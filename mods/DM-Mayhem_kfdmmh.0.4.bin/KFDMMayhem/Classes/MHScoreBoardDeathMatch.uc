class MHScoreBoardDeathMatch extends ScoreBoardDeathMatch;
...
// DM Mayhem modification
/*
			if ( GRI.PRIArray[i].bOutOfLives )
				Canvas.DrawText(OutText,true);
			else
*/
				Canvas.DrawText( MHPlayerReplicationInfo( GRI.PRIArray[i] ).fragCount,true );
		}
...
*/
		...
		Canvas.DrawText( MHPlayerReplicationInfo( GRI.PRIArray[OwnerOffset] ).fragCount,true );
...
// Duplicated code at MHScoreBoardTeamDeathMatch
...
{
	local MHPlayerReplicationInfo player1,player2;

	player1 = MHPlayerReplicationInfo(p1);
	player2 = MHPlayerReplicationInfo(p2);

	if(player1 != none && player2 != none)
	{
		if( player1.bOnlySpectator )
		{
			if( player2.bOnlySpectator )
				return true;
			else
				return false;
		}
		else if ( player2.bOnlySpectator )
			return true;

		if( player1.fragCount < player2.fragCount )
			return false;
		if( player1.fragCount == player2.fragCount )
		{
