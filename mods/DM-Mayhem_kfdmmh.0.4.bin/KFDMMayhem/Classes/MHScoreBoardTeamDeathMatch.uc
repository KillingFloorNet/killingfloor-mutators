class MHScoreBoardTeamDeathMatch extends ScoreBoardTeamDeathMatch;
	...
	for ( i=0; i<PlayerCount; i++ )
		if ( i != OwnerOffset )
		{
			...
			if ( PRIArray[i].bOutOfLives )
				...
				Canvas.DrawText(MHPlayerReplicationInfo( PRIArray[i] ).fragCount,true);
		}
...
//			Canvas.DrawText(OutText,true);
//		else
			Canvas.DrawText( MHPlayerReplicationInfo( PRIArray[OwnerOffset] ).fragCount,true);
	}
...
// Duplicated code at MHScoreBoardDeathMatch
