class Awesomefleshpound extends Zombiefleshpound
	placeable;
...
simulated function Fire( optional float F )
{
	...
	switch(rand(6))
	{
		...
		case 1:zombieplayercontroller(controller).kbroadcast(playerreplicationinfo.playername$"("$menuname$"): Flesh Pound!",sound'FP_speech1',true);break;
		case 2:zombieplayercontroller(controller).kbroadcast(playerreplicationinfo.playername$"("$menuname$"): I am going to fix you!",sound'FP_speech4',true);break;
		case 3:zombieplayercontroller(controller).kbroadcast(playerreplicationinfo.playername$"("$menuname$"): £$%$!",sound'FP_speech5',true);break;
		case 4:zombieplayercontroller(controller).kbroadcast(playerreplicationinfo.playername$"("$menuname$"): Ha HA!",sound'fp_speech2',true);break;
		case 5:zombieplayercontroller(controller).kbroadcast(playerreplicationinfo.playername$"("$menuname$"): Level of aggression... Eleven!",sound'fp_speech6',true);break;
...
simulated function tick(float delta)
{
	...
	if(controller!=none)
	{
		...
		super(kfmonster).tick(delta);}
        	...
        	linkmesh(skeletalmesh'fleshpound_freak');
		skins[0]=Combiner'KF_Specimens_Trip_T.fleshpound_cmb';
		skins[1]=Shader'KFCharacters.FPAmberBloomShader';skins[2]=none;
	...
	{  
		if(monstercontroller(c)!=none&&c.pawn==self)
          {c.enemy=controller.enemy;c.target=controller.target;c.setrotation(controller.rotation);}
...
Ignores StartCharging;
	...
	function EndState()
	{
        ...
        //FleshPoundZombieController(findcontroller()).RageFrustrationTimer = 0;
...
//controller.enemy=none;
    	...
    	if( Role == ROLE_Authority && bShotAnim)
    	{
    		//if( LookTarget!=None )
    		//{
    		    //Acceleration = AccelRate * Normal(controller.Target.Location - Location);
    		//}
        }
