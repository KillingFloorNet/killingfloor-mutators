class zBoardTeamDeathMatch extends ScoreBoardDeathMatch;
...
// end if _RO_
	...
	for ( i=0; i<PlayerCount; i++ )
		if ( i != OwnerOffset )
		{
			...
			if ( PRIArray[i].bOutOfLives )
				...
				Canvas.DrawText(kfplayerreplicationinfo(PRIArray[i]).kills,true);
		}
	...
	if ( OwnerOffset >= 0 )
	{
		...
		if ( PRIArray[OwnerOffset].bOutOfLives )
			...
			Canvas.DrawText(kfplayerreplicationinfo(PRIArray[OwnerOffset]).kills,true);
	}
