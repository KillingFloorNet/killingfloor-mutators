...
//=============================================================================
class SentryGun extends KFWeapon;

var Sentry CurrentSentry;
...
event ServerStartFire(byte Mode)
{
	...
	if( CurrentSentry==None )
	{
		...
		{
			CurrentSentry = Spawn(Class'Sentry',,,Spot,R);
			if( CurrentSentry!=None )
			{
				if( PlayerController(Instigator.Controller)!=None )
					PlayerController(Instigator.Controller).ReceiveLocalizedMessage(Class'SentryMessage',1);
		...
		if( PlayerController(Instigator.Controller)!=None )
			PlayerController(Instigator.Controller).ReceiveLocalizedMessage(Class'SentryMessage',0);
	}
}
...
defaultproperties
{
     ...
     InventoryGroup=5
     GroupOffset=6
     PickupClass=Class'Doom3KFBeta4.SentryGunPickup'
     PlayerViewOffset=(X=25.000000,Z=-25.000000)
     BobDamping=6.000000
     AttachmentClass=Class'Doom3KFBeta4.SentryGunAttachment'
