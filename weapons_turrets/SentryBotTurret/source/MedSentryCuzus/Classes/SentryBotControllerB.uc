// Armor Sentry Bot
class SentryBotControllerB extends AIController;

var SentryBotTurret Sentry;
var bool bLostContactToPL;

function Restart()
{
	Enemy = None;
	Sentry = SentryBotTurret(Pawn);
	GoToState('WakeUp');
}
...
{
	if( Other==None || Other.ShieldStrength<=0 || Other.Controller==None || Other==Enemy )
		return;
	...
	if( KFHumanPawn(Other)==None )
		return;
	//
	if( Enemy!=None && Enemy.ShieldStrength<=0)
		Enemy = None;
	//Flame
...
final function GoNextOrders()
{
	...
	if( Sentry.OwnerPawn==None || Sentry.OwnerPawn.ShieldStrength<=0 )
	{
		Sentry.OwnerPawn = None;
		Sentry.PlayerReplicationInfo = None;
	}
	if( Enemy!=None && Enemy.ShieldStrength>=0 && (Sentry.OwnerPawn==None || LineOfSightTo(Sentry.OwnerPawn)) )
	{
...
Begin:
	if( Enemy==None || Enemy.ShieldStrength<=0 || Enemy.ShieldStrength>=100) //Flame
	{
BadEnemy:
		Enemy = None;
		GoNextOrders();
...
ShootEnemy:
	...
	while	(
				...
				&& Enemy.ShieldStrength < 100 //Flame
				&& LineOfSightTo(Enemy)
				&& (Sentry.OwnerPawn == None || LineOfSightTo(Sentry.OwnerPawn))
			)
	{
		Pawn.Acceleration = vect(0,0,0);
