class SentryBotTurret 
	extends Pawn 
	config(SentryBotTurret);
...
var Emitter mTracer;
var SentryBotFireFlash WeaponFlash;
var transient float NextVoiceTimer;
var SentryBotGun WeaponOwner;
var transient Font HUDFontz[2];
//Flame
var float VetC;
//

var byte SentryMode; // 0 Health, 1 Armor
...
final function SetOwningPlayer( Pawn Other, SentryBotGun W )
{
	//Flame
	...
	if(KFPRI!=none && KFPRI.ClientVeteranSkill.default.PerkIndex==0)
		VetC=1.0 + KFPRI.ClientVeteranSkillLevel * 0.2;
	else
		VetC=1.0;
	//
}
...
simulated event PostBeginPlay()
{
	...
	//Flame
	VetC = 1.0;
	// Log("PostBeginPlay.0"@VetC);
	//

	if (Role == Role_Authority) //Flame
	{
		TweenAnim(IdleRestAnim,0.01);

		if (SentryMode == 1)
		{
			ControllerClass = Class'SentryBotControllerB';
		}
...
simulated function SetAnimationNum( byte Num )
{
	...
	case 1:
		...
		break;
	case 2:
		LoopAnim('Ranged_Attack1',1.6f * VetC);//Flame
		if( Level.NetMode!=NM_Client )
			SetTimer(1.00 / VetC,true);
		//Flame
		/*
		if( Level.NetMode!=NM_DedicatedServer )
		{
			Log("SetAnimationNum.0"@Level.TimeSeconds);
			WeaponFlash = Spawn(Class'SentryBotFireFlash',Self);
...
simulated function Timer()
{
	...
	if(A!=None)// && KFPawn(A)==None )
		DoMedicFireEffect();
		//A.TakeDamage(HitDamage,OwnerPawn,HL,X*1000.f,Class'DamTypeSentryFire');
	else if( A==None )
...
function DoMedicFireEffect()
{
	local Vector StartFireLocation, EndFireLocation, DVector;
	local Rotator AimRot;

	if(Controller!=none && Controller.Enemy!=none)
	{
		StartFireLocation=GetBoneCoords('Barrel').Origin;
		EndFireLocation=Controller.Enemy.GetBoneCoords('CHR_Spine1').Origin;
		DVector=EndFireLocation-StartFireLocation;
		AimRot=Rotator(DVector);
	}
	if (SentryMode == 1)
	{
		// Armor
		Spawn(Class'SentryBotHealingProjectileB',self,, StartFireLocation, AimRot);
	}
	else
	{
		Spawn(Class'SentryBotHealingProjectile',self,, StartFireLocation, AimRot);
	}
}
...
simulated final function ClientTraceFX()
{
	...
	local Actor A;
	//Flame
	local SentryBotFireFlash flash;
	//
	...
	TraceFX(A,HL,HN);
	//Flame
	flash=Spawn(Class'SentryBotFireFlash',Self);//,,GetBoneCoords('Barrel').Origin);
	AttachToBone(flash, 'Barrel');
	//Log("SentryBotFireFlash.0"@flash@flash.Location@GetBoneCoords('Barrel').Origin);
	/*
	if(WeaponFlash==none)
	{
		WeaponFlash = Spawn(Class'SentryBotFireFlash',Self);
...
function Died(Controller Killer, class<DamageType> damageType, vector HitLocation)
{
	...
	if( WeaponOwner!=None )
	{
		if( OwnerPawn!=None && PlayerController(OwnerPawn.Controller)!=None )
			PlayerController(OwnerPawn.Controller).ReceiveLocalizedMessage(Class'SentryBotMessage',2);
...
simulated function HurtRadius( float DamageAmount, float DamageRadius, class<DamageType> DamageType, float Momentum, vector HitLocation )
{
	...
	foreach CollidingActors (class 'Actor', Victims, DamageRadius, HitLocation)
	{
		...
		&& ExtendedZCollision(Victims)==None && KFPawn(Victims)==None && SentryBotTurret(Victims)==None )
		{
...
{
	hitdamage=7
	SentryHealth=400 //250

	FootStep(0)=Sound'SentryBot_A.Sentry.Sentry_sstep_01'
	FootStep(1)=Sound'SentryBot_A.Sentry.Sentry_sstep_02'
	FootStep(2)=Sound'SentryBot_A.Sentry.Sentry_sstep_03'
	FootStep(3)=Sound'SentryBot_A.Sentry.Sentry_sstep_04'
	VoicesList(0)=Sound'SentryBot_A.Sentry.Sentry_activate_01'
	VoicesList(1)=Sound'SentryBot_A.Sentry.Sentry_cant_reach_player_01'
	VoicesList(2)=Sound'SentryBot_A.Sentry.Sentry_cant_reach_player_02'
	VoicesList(3)=Sound'SentryBot_A.Sentry.Sentry_destroyed_01'
	VoicesList(4)=Sound'SentryBot_A.Sentry.Sentry_fight_enemy_02'
	VoicesList(5)=Sound'SentryBot_A.Sentry.Sentry_sight_enemy_01'
	VoicesList(6)=Sound'SentryBot_A.Sentry.Sentry_lost_target_01'
	VoicesList(7)=Sound'SentryBot_A.Sentry.Sentry_pain_01'
	VoicesList(8)=Sound'SentryBot_A.Sentry.Sentry_pain_02'
	VoicesList(9)=Sound'SentryBot_A.Sentry.Sentry_pain_03'
	VoicesList(10)=Sound'SentryBot_A.Sentry.Sentry_pain_04'
	VoicesList(11)=Sound'SentryBot_A.Sentry.Sentry_shutdown_01'
	VoicesList(12)=Sound'SentryBot_A.Sentry.Sentry_sight_friendly_01'
	VoicesList(13)=Sound'SentryBot_A.Sentry.Sentry_wait_for_player_01'
	VoicesList(14)=Sound'SentryBot_A.Sentry.Sentry_wait_for_player_02'
	VoicesList(15)=Sound'SentryBot_A.Sentry.Sentry_wait_for_player_03'
	FiringSounds(0)=Sound'SentryBot_A.Sentry.Sentry_fire_01'
	FiringSounds(1)=Sound'SentryBot_A.Sentry.Sentry_fire_02'
	FiringSounds(2)=Sound'SentryBot_A.Sentry.Sentry_fire_03'
	FiringSounds(3)=Sound'SentryBot_A.Sentry.Sentry_fire_04'
	FiringSounds(4)=Sound'SentryBot_A.Sentry.Sentry_fire_05'
	...
	ControllerClass=Class'SentryBotController'
	...
	PrePivot=(Z=-5.000000)
	Skins(0)=Texture'SentryBot_A.Sentry.SentrySpistonDiffuse'
	Skins(1)=Texture'SentryBot_A.Sentry.SentryFlapDiffuse'
	Skins(2)=Texture'SentryBot_A.Sentry.SentrySkin2'
	Skins(3)=Shader'SentryBot_A.Sentry.InvisibleWeaponsFlash'
	Skins(4)=Shader'SentryBot_A.Sentry.InvisibleWeaponsFlash'
	Skins(5)=Shader'SentryBot_A.Sentry.InvisibleWeaponsFlash'
	Skins(6)=Shader'SentryBot_A.Sentry.InvisibleWeaponsFlash'
	Skins(7)=Shader'SentryBot_A.Sentry.InvisibleWeaponsFlash'
	Skins(8)=Shader'SentryBot_A.Sentry.InvisibleWeaponsFlash'
