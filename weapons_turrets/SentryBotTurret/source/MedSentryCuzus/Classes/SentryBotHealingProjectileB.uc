// Armor Sentry Bot
class SentryBotHealingProjectileB extends HealingProjectile;
...
simulated function ProcessTouch(Actor Other, Vector HitLocation)
{
  ...
  || Other.Base == Instigator)
    ...
    && Healed.ShieldStrength < 100)
      ...
      if (Healed.ShieldStrength + HealSum > 100)
      {
        MedicReward = 100 - (Healed.ShieldStrength + HealSum);
        if (MedicReward < 0)
        {
          MedicReward = 0;
        }
...
function AddDamagedHealStats(int MedicReward)
{
	...
	{
		KFSteamStats.AddWeldingPoints(MedicReward);
	}
}
