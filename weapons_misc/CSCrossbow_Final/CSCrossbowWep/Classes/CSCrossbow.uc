...
class CSCrossbow extends KFWeapon;
...
#exec OBJ LOAD FILE="CSCrossbow_A.ukx"
#exec OBJ LOAD FILE=..\textures\KF_Weapons_Trip_T.utx
...
static function PreloadAssets(Inventory Inv, optional bool bSkipRefCount)
{
	...
	default.ScriptedTextureFallback = Combiner(DynamicLoadObject(default.ScriptedTextureFallbackRef, class'Combiner', true));

	if ( CSCrossbow(Inv) != none )
	{
		CSCrossbow(Inv).ZoomMat = default.ZoomMat;
		CSCrossbow(Inv).ScriptedTextureFallback = default.ScriptedTextureFallback;
	}
}
...
simulated function UpdateScopeMode()
{
		...
		Instigator.IsHumanControlled() )
    {
	    if( KFScopeDetail == KF_ModelScope )
		{
			...
			{
				// templog"Initializing ScopeScriptedTexture...");
			...
			{
				// templog"Initializing ScopeScriptedCombiner...");
				// Construct the Combiner
				ScriptedScopeCombiner = Combiner(Level.ObjectPool.AllocateObject(class'Combiner'));
	            ScriptedScopeCombiner.Material1 = Texture'CSCrossbow_A.cscrossbow_cross';
			...
			{
				// templog"Initializing ScopeScriptedShader...");
		...
		else if( KFScopeDetail == KF_ModelScopeHigh )
		{
			...
			if( ScopeScriptedTexture == none )
			{
				// templog"Initializing ScopeScriptedTexture 2...");
	        	ScopeScriptedTexture = ScriptedTexture(Level.ObjectPool.AllocateObject(class'ScriptedTexture'));
	        }
			ScopeScriptedTexture.FallBackMaterial = ScriptedTextureFallback;
			...
			if( ScriptedScopeCombiner == none )
			{
				// templog"Initializing ScopeScriptedCombiner 2...");
				// Construct the Combiner
				ScriptedScopeCombiner = Combiner(Level.ObjectPool.AllocateObject(class'Combiner'));
	            ScriptedScopeCombiner.Material1 = Texture'CSCrossbow_A.cscrossbow_cross';
	            ScriptedScopeCombiner.FallbackMaterial = Shader'ScopeShaders.Zoomblur.LensShader';
			...
			if( ScopeScriptedShader == none )
			{
				// templog"Initializing ScopeScriptedShader 2...");
	            // Construct the scope shader
				ScopeScriptedShader = Shader(Level.ObjectPool.AllocateObject(class'Shader'));
				ScopeScriptedShader.Diffuse = ScriptedScopeCombiner;
				ScopeScriptedShader.SelfIllumination = ScriptedScopeCombiner;
...
simulated event RenderOverlays(Canvas Canvas)
{
	...
	PreDrawFPWeapon();	// Laurent -- Hook to override things before render (like rotation if using a staticmesh)
 	...
 	if(bAimingRifle && PC != none && (KFScopeDetail == KF_ModelScope || KFScopeDetail == KF_ModelScopeHigh))
 	{
 		if (ShouldDrawPortal())
 		{
			if ( ScopeScriptedTexture != none )
			{
				// templog"We have ScopeScriptedTexture...");
	...
	{
		if (ScriptedTextureFallback != none)
		{
		Skins[LenseMaterialID] = ScriptedTextureFallback;
		}
		else
		{
		// templog"NO SCRIPTEDTEXTUREFALLBACK...");
		}
 	...
 	else
 	{
		if (ScriptedTextureFallback != none)
		{
		// templog"CHECK: SCRIPTEDTEXTUREFALLBACK IS HERE...");
		Skins[LenseMaterialID] = ScriptedTextureFallback;
		}
		else
		{
		// templog"CHECK: SCRIPTEDTEXTUREFALLBACK IS NONE...");
		Skins[LenseMaterialID] = default.Skins[LenseMaterialID];
		}
		bDrawingFirstPerson = true;
...
defaultproperties
{
     MagCapacity=50
     ReloadRate=2.666666
	 // ForceZoomOutOnFireTime=0.200000
     ReloadAnim="Reload"
     ...
     TraderInfoTexture=Texture'CSCrossbow_A.cs_xbow_trader'
     bIsTier2Weapon=True
     MeshRef="CSCrossbow_A.cs_xbow_full"
	 SkinRefs(0)="CSCrossbow_A.cs_crossbow_cmb"
	 SkinRefs(1)="CSCrossbow_A.cs_crossbow_cmb"
	 SkinRefs(2)="CSCrossbow_A.cs_crossbow_scope_cmb"
	 SkinRefs(3)="KF_Weapons_Trip_T.Rifles.CBLens_cmb"
     SelectSoundRef="KF_HandcannonSnd.50AE_Select"
     HudImageRef="CSCrossbow_A.cs_xbow_unsel"
     SelectedHudImageRef="CSCrossbow_A.cs_xbow_sel"
     PlayerIronSightFOV=32.000000
     ZoomedDisplayFOV=40.000000
     FireModeClass(0)=Class'CSCrossbowWep.CSCrossbowFire'
     ...
     InventoryGroup=3
     GroupOffset=5
     PickupClass=Class'CSCrossbowWep.CSCrossbowPickup'
     PlayerViewOffset=(X=25.000000,Y=17.000000,Z=-8.000000)
     BobDamping=6.000000
     AttachmentClass=Class'CSCrossbowWep.CSCrossbowAttachment'
     IconCoords=(X1=245,Y1=39,X2=329,Y2=79)
     ItemName="CS:O Crossbow"
     TransientSoundVolume=1.250000
	 
	bSniping = true
    ZoomMatRef="CSCrossbow_A.cscrossbow_scope_fb"
