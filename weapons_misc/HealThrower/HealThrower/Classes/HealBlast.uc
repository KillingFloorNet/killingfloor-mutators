class HealBlast extends KFShotgunFire;
...
static function PreloadAssets(LevelInfo LevelInfo, optional KFShotgunFire Spawned)
{
	...
	if ( HealBlast(Spawned) != none )
	{
		HealBlast(Spawned).FireEndSound = default.FireEndSound;
		HealBlast(Spawned).AmbientFireSound = default.AmbientFireSound;
	}
}
...
function DoFireEffect()
{
    ...
    local HealCloud FiredRockets[4];
	...
	for ( p=0; p<SpawnCount; p++ )
	{
		...
		FiredRockets[p] = HealCloud(SpawnProjectile(FireLocation, Aim));
	}
}
...
defaultproperties
{
     ...
     FireSoundRef="none"
     FireRate=0.07
     AmmoClass=Class'HHealThrowerAmmo'
	 ProjectileClass=Class'HealCloud'
