...
// FLight.
//=============================================================================
class FLight extends KFWeapon
	Config(KFFloodLight);

var FloodLight CurrentSentry;
	...
	if( CurrentSentry==None )
	{
		...
		if( FastTrace(Spot,Instigator.Location) )
		{
			if( bStationaryTurret )
				CurrentSentry = Spawn(Class'FloodLightStationary',,,Spot,R);
			else CurrentSentry = Spawn(Class'FloodLight',,,Spot,R);
			if( CurrentSentry!=None )
			{
				if( PlayerController(Instigator.Controller)!=None )
					PlayerController(Instigator.Controller).ReceiveLocalizedMessage(Class'FloodLightMessage',1);
		...
		if( PlayerController(Instigator.Controller)!=None )
			PlayerController(Instigator.Controller).ReceiveLocalizedMessage(Class'FloodLightMessage',0);
	}
}
...
defaultproperties
{
    ...
    InventoryGroup=5
    GroupOffset=6
    PickupClass=Class'FloodLightPickup'
    PlayerViewOffset=(X=25,Y=0.000000,Z=-25)
    //PlayerViewPivot=(Pitch=400)
    BobDamping=6.000000
    AttachmentClass=Class'FloodLightAttachment'
	...
	//HudImage=Texture'FloodLightDT_T.FloodLight_unselected'
	//SelectedHudImage=Texture'FloodLightDT_T.FloodLight_Selected'
	TraderInfoTexture=Texture'FloodLightDT_T.FloodLight_Trader'
	bCanThrow=false
}