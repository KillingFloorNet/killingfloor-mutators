...
class ZombieKf2Boss extends ZombieKf2BossBase;
...
var Kf2BossHPNeedle CurrentNeedle;
...
function PatriarchKnockDownNew()
{
//    PlaySound(SoundGroup'KF2Patriarh.Patrick.snd.Help', SLOT_Misc, 2.0,true,500.0);
}

function PatriarchEntranceNew()
{
    PlaySound(SoundGroup'KF2Patriarh.Patrick.snd.Entrance', SLOT_Misc, 2.0,true,500.0);
}

function PatriarchVictoryNew()
{
//    PlaySound(SoundGroup'DZMonsterPack.Patrick_G.snd.VictoryP', SLOT_Misc, 2.0,true,500.0);
}
...
function PatriarchRadialTauntNew()
{
    if( NumNinjas > 0 && NumNinjas > NumLumberJacks )
    {
        PlaySound(SoundGroup'KF2Patriarh.Patrick.snd.DieGr', SLOT_Misc, 2.0,true,500.0);
    }
...
simulated function CloakBoss()
{
	...
	Skins[2] = Shader'KF_Specimens_Trip_T.patriarch_invisible';
...
event Bump(actor Other)
{
	...
	{ // Kill the annoying deco brat.
		...
		SetAnimAction(MeleeAnims[Rand(5)]);
		//PlaySound(sound'Claw2s', SLOT_None);  KFTODO: Replace this
		HandleWaitForAnim(MeleeAnims[Rand(5)]);
	}
}
...
Ignores RangedAttack;
	...
	function AnimEnd( int Channel )
	{
		...
		Start2 = GetBoneCoords('LeftMissile120').Origin;
		Start3 = GetBoneCoords('LeftMissile240').Origin;	
		...
		L = AdjustAim(SavedFireProperties,Start2,100);
		B = AdjustAim(SavedFireProperties,Start3,100);
		PlaySound(RocketFireSound,SLOT_Interact,2.0,,TransientSoundRadius,,false);
		Spawn(Class'BossLAWProj',,,Start,R);
		Spawn(Class'BossLAWProj',,,Start2,L);
		Spawn(Class'BossLAWProj',,,Start3,B);
		...
		if ( FRand() < 0.10 && Controller.Enemy != none && PlayerController(Controller.Enemy.Controller) != none )
		{
...
Begin:
    if( Health > 0 )
    {
    	...
    	CloakBoss();
    	PlaySound(sound'KF2Patriarh.Patrick.snd.help_child', SLOT_Misc, 2.0,,500.0);
...
Begin:
	...
	While( true )
	{
		...
		if( SneakCount > 1000 || (Controller != none && Kf2BossZombieController(Controller).bAlreadyFoundEnemy) )
		{
            GoToState('');
		}
...
simulated function DropNeedle()
{
	if( CurrentNeedle!=None )
	{
		...
		CurrentNeedle.SetLocation(GetBoneCoords('SyringeAttach01').Origin);
...
simulated function NotifySyringeA()
{
	...
	if( Level.NetMode!=NM_DedicatedServer )
	{
		...
		AttachToBone(CurrentNeedle,'Syringe');
...
{
	if( AnimName=='MeleeImpale' || AnimName=='MeleeClaw' || AnimName=='transition' || AnimName=='MeleeNoga' || AnimName=='MeleePlecho' )
	{
		AnimBlendParams(1, 1.0, 0.0,, SpineBone1);
...
simulated event SetAnimAction(name NewAction)
{
    ...
    if( Controller != none )
    {
	   Kf2BossZombieController(Controller).AnimWaitChannel = ExpectingChannel;
	}
...
simulated function HandleWaitForAnim( name NewAnim )
{
    ...
    Kf2BossZombieController(Controller).SetWaitForAnimTimout(RageAnimDur,NewAnim);
}
...
defaultproperties
{
     ControllerClass=Class'Kf2BossZombieController'
}
