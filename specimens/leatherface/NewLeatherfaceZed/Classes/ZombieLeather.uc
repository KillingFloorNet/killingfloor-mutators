...
class ZombieLeather extends ZombieLeatherBase;

#exec OBJ LOAD FILE=Leatherface_T.utx
...
simulated function PostBeginPlay()
{
	...
	SpawnExhaustEmitterLeft();

	SpawnExhaustEmitterRight();
}
...
simulated function SpawnExhaustEmitterLeft()
{
...
simulated function SpawnExhaustEmitterRight()
{
	if ( Level.NetMode != NM_DedicatedServer )
	{
		if ( ExhaustEffectClass != none )
		{
			...
			if ( ExhaustEffect != none )
			{
				AttachToBone(ExhaustEffect, 'Chainsaw_lod2');
				ExhaustEffect.SetRelativeLocation(vect(0, -20, 0));
			}
		}
	}
...
{//should be derived and used.
	...
	myLevel.AddPrecacheMaterial(Texture'Leatherface_T.Hair');
	myLevel.AddPrecacheMaterial(Texture'Leatherface_T.body');
	myLevel.AddPrecacheMaterial(Texture'Leatherface_T.Eyes');
}

defaultproperties
{
     EventClasses(0)="KFChar.ZombieScrake_STANDARD"
     EventClasses(1)="KFChar.ZombieScrake_STANDARD"
