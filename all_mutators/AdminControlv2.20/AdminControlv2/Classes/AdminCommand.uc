...
static function CommandMessage(AdminRecord Sender, string Mess, optional Controller Target, optional bool bIgnoreSilentMode)
{
	local Controller C;
	...
	if ( Target == none )
	{
		for( C = Sender.Level.ControllerList; C != None; C = C.nextController )
		{ 
			if( PlayerController(C) != none && ( PlayerController(C) == Sender.Controller || !Sender.bSilentMode || bIgnoreSilentMode ) ) 
			{
				PlayerController(C).ClientMessage(Mess);
			}
		}
	}
	else if ( PlayerController(Target) == Sender.Controller || !Sender.bSilentMode || bIgnoreSilentMode )
	{
