/*
 * 23.05.2016 WhatEver fix
 */
class ServerAdsRev extends Info config;
...
function PostBeginPlay()
{
	...
	if (!bInitialized)
	{
		...
		log("<ServerAdsKF> There are "$nLines$" lines in the list");
...
event Timer()
{
	...
	if ((iCurPos >= nLines) && (bWrapAround == false))
		return;
...
function getLinesFromWeb()
{
	...
	if (DEBUG)
		log("[D] ServerAdsKF Download lines from the web");
