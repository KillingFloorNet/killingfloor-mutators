...
simulated function Explode(vector HitLocation, vector HitNormal)
{
	...
	if (ROLE == ROLE_Authority)
	{
		foreach CollidingActors (class 'Actor', A, DamageRadius, HitLocation)
		{
			...
			{
				K = KFMonster(A);
				p = Instigator.Spawn(Class'WTFEquipNadeStunProj', instigator,, Location, K.Rotation);
    ...
    {
        Spawn(Class'WTFEquipNadeExplosionEmitter',,,HitLocation + HitNormal*20,rotator(HitNormal));
		Spawn(Class'WTFEquipNadeStunFlash',,,HitLocation + HitNormal*20,rotator(HitNormal));
    }
