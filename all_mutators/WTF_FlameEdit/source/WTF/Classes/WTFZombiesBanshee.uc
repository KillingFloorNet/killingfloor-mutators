...
function PlayScreamSound()
{
	...
	{
		PlaySound(sound'WTF_A.BansheeScream', SLOT_Pain,1.30,true,525);
	}
}
...
simulated function SpawnTwoShots()
{
	...
	//PlaySound(sound'WTF_A.BansheeScream', SLOT_Interact);
...
simulated function Tick(float DeltaTime)
{
	...
	if( Level.TimeSeconds > NextCheckTime && Health > 0 )
	{
		...
		if ( !bSpotted && !bCloaked && Skins[0] != Texture'WTF_A.WTFZombies.Banshee') //&& Skins[0] != Combiner'KF_Specimens_Trip_T.stalker_cmb' 
		{
			UncloakStalker();
		}
...
simulated function UnCloakStalker()
{
	if( !bCrispified )
	{
		...
		if ( Skins[0] !=  Texture'WTF_A.WTFZombies.Banshee') //Combiner'KF_Specimens_Trip_T.stalker_cmb'
		{
			Skins[1] = FinalBlend'KF_Specimens_Trip_T.stalker_fb';
			Skins[0] = Texture'WTF_A.WTFZombies.Banshee';
