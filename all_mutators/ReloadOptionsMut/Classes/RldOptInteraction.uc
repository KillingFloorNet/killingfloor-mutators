class RldOptInteraction extends Interaction
	config(ReloadOptionsMut);

var ReloadOptionsMut Mut;
...
function RegisterMutator(ReloadOptionsMut aMut) {
	Mut = aMut;
	SaveConfig();
}
...
function ClearReloadMessages(optional bool bSwitchedModes) {
	...
	if (PC.Level.timeSeconds - timeReceivedMessage > class'ReloadOptionsMut.RldOptMessages'.default.lifeTime)
		return;

	W = GetWeapon();
	if (W != None && (W.ReadyToFire(0) || bSwitchedModes) && AltFireSwitchesModes(W)) {
		PC.ReceiveLocalizedMessage(class'ReloadOptionsMut.RldOptMessages', 3);
...
function SendReloadMessage(bool bShouldReceive, int aSwitch) {
	if (bShouldReceive) {
		...
		PC.ReceiveLocalizedMessage(class'ReloadOptionsMut.RldOptMessages', aSwitch);
...
function ShowMenu() {
	...
	local KFInvasionLoginMenu IMenu;
	local RldOptMidGameOptions Panel;

	GC = KFGUIController(ViewportOwner.GUIController);
	if (GC.ActivePage == None)
		PC.ShowMenu();

	IMenu = KFInvasionLoginMenu(GC.ActivePage);
	if (IMenu != None && IMenu.c_Main.TabIndex(NewTab.Caption) == -1) {
		Panel = RldOptMidGameOptions(IMenu.c_Main.AddTabItem(NewTab));
		Panel.MyInteraction = Self;
		if (Panel != None) {
			Panel.ModifiedChatRestriction = IMenu.UpdateChatRestriction;
...
defaultproperties
{
     NewTab=(ClassName="ReloadOptionsMut.RldOptMidGameOptions",Caption="Reload Options",Hint="Options to customize reload.")
