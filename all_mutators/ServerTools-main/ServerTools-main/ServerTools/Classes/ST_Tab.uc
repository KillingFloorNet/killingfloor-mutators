...
function ShowPanel(bool bShow)
{
  ...
  if (bShow)
  {
    ...
    if (MutRef.OnlyVoteTraderGUI)
    {
      ...
      if(!MutRef.AdminAndSelectPlayers) DisableComponent(b_ApplyButton);
      if(!MutRef.AdminAndSelectPlayers) DisableComponent(ed_CurrentTrader);
      if(!MutRef.AdminAndSelectPlayers) DisableComponent(b_ApplyButton2);
