class CashPickupDoshWeapon extends CashPickup;
...
state FallingPickup
{
    function Touch(actor Other)
	{
		...
		else if (KFMonster(Other) != none && KFMonster(Other).Health > 0 && ZombieBoss(Other) == none)
		{
            KFMonster(Other).TakeDamage(CashAmount * damageScale, KFHumanPawn(DroppedBy.Pawn), Location, Dummy, DoshDamType);
            SetRespawn();
        }
...
{
    damageScale=1.0;
    DoshDamType=class'DamTypeDoshWeapon'
}
