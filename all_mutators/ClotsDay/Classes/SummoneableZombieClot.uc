class SummoneableZombieClot extends ZombieClot;
...
replication
{
	reliable if (bNetInitial && (Role < ROLE_Authority)) cloakStateChangeTime;
	reliable if (Role == ROLE_Authority) nextCloakStateChangeTime;
}

simulated function PostBeginPlay()
{
	if( level.netMode != NM_DedicatedServer ) cloak();
	super.PostBeginPlay();
	if( level.netMode != NM_DedicatedServer ) self.setNextCloakStateChangeTime();
}
...
simulated function uncloak()
{
	...
	bCloaked = false;
	bUnlit = false;
	if ( Skins.Length > 1 ) Skins.Remove(1,1);
...
simulated function tick( float deltaTime )
{
	...
	if (Level.NetMode == NM_DedicatedServer) return;
	if( bCloaked && Level.TimeSeconds > nextCloakStateChangeTime ) self.nextCloakedState();
}
...
state semiInvisibleCloak
{
	...
	{
		self.semiUncloak();
		self.setNextCloakStateChangeTime();
	}
}
