...
function Timer()
{
	...
	if( MO==None )
		return;
//	if( Class<DoomMonster>(MO.MonsterClass)!=None )
//	{
//		Class<DoomMonster>(MO.MonsterClass).Default.bIsBossSpawn = true;
//		M = Spawn(MO.MonsterClass);
//		Class<DoomMonster>(MO.MonsterClass).Default.bIsBossSpawn = false;
//	}
//	else
		M = Spawn(MO.MonsterClass);
	GT = BSGameType(Level.Game);
	if( M==None && bNotifyGame )
	{
		GT.MidBossAddMonsterFailed(MO);
		return;
	}
//	if( DoomMonster(M)!=None )
//		DoomMonster(M).NotifyTeleport();
	if( bNotifyGame )
		GT.MidBossAddMonsterSuccess(MO);
	if( Level.NetMode==NM_DedicatedServer )
		Destroy();
}
