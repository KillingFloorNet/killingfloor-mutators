...
simulated function Explode(vector HitLocation, vector HitNormal)
{
	...
	if (ROLE == ROLE_Authority)
	{
		foreach CollidingActors (class 'Actor', A, DamageRadius, HitLocation)
		{
			...
			{
				K = KFMonster(A);
				p = Instigator.Spawn(Class'WTF.WTFEquipNadeStunProj', instigator,, Location, K.Rotation);
    ...
    {
        Spawn(class'WTF.WTFEquipNadeExplosionEmitter',,,HitLocation + HitNormal*20,rotator(HitNormal));
		Spawn(class'WTF.WTFEquipNadeStunFlash',,,HitLocation + HitNormal*20,rotator(HitNormal));
    }
