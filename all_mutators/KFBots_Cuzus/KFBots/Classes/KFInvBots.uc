...
var KFBotsUseTrigger UseNotify;
...
{
	super.PostBeginPlay();
//	UseNotify = Spawn(Class'KFBotsUseTrigger', Self);
	AssignPersonality();
}
...
final function Actor GetRandomDest()
{
	...
	for (i=0; i<DoorPaths.Length; ++i)
	{
		if( DoorPaths[i].Door.bSealed && !DoorPaths[i].Door.bDoorIsDead )
			...
			DoorPaths.Remove(i--, 1); // Remove opened paths.
	}
...
function Possess(Pawn aPawn)
{
	...
	if (Vehicle(Pawn) == none && Pawn != None)
	{
		...
		if(xPawn(Pawn) != None)
		{
			xPawn(Pawn).bCanDoubleJump = false; //true; // To stop them from getting stuck at jumps only zeds can make.
			xPawn(Pawn).MaxMultiJump = 0; //1;
			xPawn(Pawn).MultiJumpRemaining = 0; //1;
		}
	}
}
		...
		else // buy that gun
		{
			...
			if( Weap!=None )
			{
				if ( KFGameType(Level.Game) != none )
					KFGameType(Level.Game).WeaponSpawned(Weap);
...
function bool EnemyReallyScary()
{
	...
	// 2020-01-12 19:42:42 (Function KFInvBots.EnemyReallyScary:00AB) Runaway loop detected (over 10000000 iterations)
	/*
	local Controller C;
	local KFMonster M;
	local byte i;
...
function FightEnemy(bool bCanCharge, float EnemyStrength)
{
	...
	if( NextNadeTimer<Level.TimeSeconds && MyGrenades!=None && MyGrenades.HasAmmo() && Rand(3)==0 && ShouldNadeEnemy() )
	{
		...
		NextNadeTimer = Level.TimeSeconds+1.f+FRand()*4.f;
		...
		for ( P = Level.ControllerList; P != none; P = P.NextController )
		{
			if( P.bIsPlayer && KFPlayerController(P)!=None )
			{
				if( BroadcastType=='Local' )
					...
					KFPlayerController(P).ClientLocationalVoiceMessage(Sender, Recipient, messagetype, messageID, soundSender, senderLocation);
			}
		}
	}
}
...
function SendMessage(PlayerReplicationInfo Recipient, name MessageType, byte MessageID, float Wait, name BroadcastType)
{
	if (ValidVoice(MessageType))
	{
		...
		// 	&& (Level.TimeSeconds - OldMessageTime < Wait + 30.f) )
		// 	return;

		if (Level.TimeSeconds - OldMessageTime < Wait + 30.f)
			return;
...
event SeePlayer(Pawn SeenPlayer)
{
	...
	else if( Enemy==None && WeldAssistTimer<Level.TimeSeconds && KFPawn(SeenPlayer)!=None && SeenPlayer.Health>0 && SeenPlayer.IsHumanControlled() && VSizeSquared(SeenPlayer.Location-Pawn.Location)<640000.f && Welder(SeenPlayer.Weapon)!=None && ActiveWelder!=None && !IsInState('Shopping') )
...
function bool FindInjuredAlly()
{
	...
	for( c=level.ControllerList; c!=none; c=c.nextController )
	{
		...
		if( aKFHPawn==none || aKFHPawn.Health<=0 || (aKFHPawn.Health+aKFHPawn.HealthToGive)>=GetMinHealingValue() || VSizeSquared(aKFHPawn.Location-Pawn.Location)>1000000.f )
			continue;
...
{
	return (FindMedGun() && MyMedGun.GetFireMode(0).AllowFire() && MyMedGun.ChargeBar()>0.49f && InjuredAlly!=none && InjuredAlly.Health>0 && (InjuredAlly.Health+InjuredAlly.HealthToGive)<GetMinHealingValue());
}
...
//
	...
	// if( PlayerReplicationInfo.Score<150 || !KFGameType(Level.Game).bWaveInProgress )
		// return false;
	//
...
{
	if( AssistingPlayer!=None && AssistingPlayer.Pawn!=None && (VSizeSquared(AssistingPlayer.Pawn.Location-Pawn.Location)>250000.f || !LineOfSightTo(AssistingPlayer.Pawn)) )
		return true;
...
// {
	...
	// function Timer()
	// {
		// if( SeekingAmmo==None || SeekingAmmo.Health<=0 || !KFGameType(Level.Game).bWaveInProgress )
		// {
