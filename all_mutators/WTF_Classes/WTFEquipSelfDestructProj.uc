...
simulated function Explode(vector HitLocation, vector HitNormal)
{
	...
	if (ROLE == ROLE_Authority)
	{
		foreach CollidingActors (class 'Actor', A, DamageRadius, HitLocation)
		{
			...
			{
				if ( !A.IsA('ZombieBoss') )
					A.TakeDamage(7000.0,Instigator,A.Location,A.Location,MyDamageType);
				else
					A.TakeDamage(7000.0,Instigator,A.Location,A.Location,MyDamageType);
			}
		}
		Instigator.TakeDamage(7000.0,Instigator,Instigator.Location,Instigator.Location,MyDamageType);
	}
