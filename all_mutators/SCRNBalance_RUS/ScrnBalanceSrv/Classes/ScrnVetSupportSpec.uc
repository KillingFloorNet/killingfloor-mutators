    ...
    if ( GetClientVeteranSkillLevel(KFPRI) > 0 ) {
                ...
                && (ClassIsChildOf(AmmoType, class'AA12Ammo') || ClassIsChildOf(AmmoType, class'KSGAmmo')) ) 
        {
            if ( GetClientVeteranSkillLevel(KFPRI) <= 6)
                ...
                return 1.5 + 0.095 * float(GetClientVeteranSkillLevel(KFPRI)-6); // +5% ammo above level 6
        }
...
static function float GetMagCapacityMod(KFPlayerReplicationInfo KFPRI, KFWeapon Other) // Количество патронов в магазине всех оружий
{
...
static function float GetReloadSpeedModifier(KFPlayerReplicationInfo KFPRI, KFWeapon Other) // Скорость перезарядки
{
	...
	if ( KFPRI.ClientVeteranSkillLevel == 20 )
			return 1.60;  // Up to 60% faster reload speed
		return 1.00;
}
...
static function float GetFireSpeedModStatic(KFPlayerReplicationInfo KFPRI, class<Weapon> Other)
{
	if ( class'ScrnBalance'.default.Mut.bHardcore )
        return 1.20; // 20% faster fire rate in hardcore mode
...
defaultproperties
{
     ...
     CustomLevelInfo="*** BONUS LEVEL %L|%s more damage with Shotguns|60% better Shotgun penetration|50% extra shotgun ammo|120% increase in grenade capacity|+%b blocks in carry weight|150% faster welding/unwelding|While holding Welder can see door healths from %wm|%d discount on Shotguns|Spawn with a AA12"
